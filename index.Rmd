...
date: "12 Eylül 2022 - Boğaziçi Üniversitesi"
output:
...
- çok kategorili (kısmi puanlama) MTK modelleri
...
```{r}
ML   <- fscores(ikipl_uyum, method="ML")
MAP  <- fscores(ikipl_uyum, method="MAP")
EAP  <- fscores(ikipl_uyum, method="EAP")
```
...
knitr::include_graphics("bilgi.PNG")
```
...
## Test Bilgi Fonksiyonu
```{r}
tinfo <- testinfo(birpl_uyum, Theta,which.items = c(1,3:5,7:10,11))
plot(Theta, tinfo, type = 'l')
```
